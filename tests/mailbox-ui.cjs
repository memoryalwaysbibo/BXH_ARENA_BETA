...
for(const token of ['const APP_VERSION = "v13.30.0"','function mailboxContext()','function renderMailboxPage()','async function handleMailbox(','data-action="mailbox-open"','callEngagementFunction("mailboxService"'])assert.ok(source.includes(token),`missing ${token}`);
