...
needle = '    <div class="panel-title" style="margin-top:20px;">稱號圖鑑</div>\n'
pos = s.find(needle)
if pos < 0:
    raise SystemExit("title gallery heading not found")
start = pos + len(needle)
end_marker = '\n  </div>`;\n}\nasync function playerDailyCheckin()'
end = s.find(end_marker, start)
if end < 0:
    raise SystemExit("title center end marker not found")
old = s[start:end]
if "titleGalleryFilter==='all'" in old:
    print("P1-B already applied")
else:
