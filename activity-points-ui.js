...
(function installActivityPointsUI(){
 ...
 function paint(){
  ...
  root.innerHTML=`<section class="ap-shell"><div class="ap-head"><div><div class="hint">玩家 ID｜${esc(playerId())}</div><h2>我的活躍積分</h2></div><button class="btn btn-ghost" data-activity="close">關閉</button></div><div class="ap-balance">${Number(w.balance||0)} <small>分</small></div><p class="hint">累積獲得 ${Number(w.lifetimeEarned||0)}｜累積扣除 ${Number(w.lifetimeDeducted||0)}</p>${w.testAccount?'<p class="hint">封測帳號可預覽介面，但不獲得活躍積分，也不列入排行榜。</p>':""}${state.error?`<p class="auth-error">${esc(state.error)}</p>`:""}<h3>我的任務</h3>${task("每日簽到","daily_checkin",15,"每日一次 +15")}${task("完成真實對戰","match_completed",30,"每場 +3")}${task("我是房主","host_completed",30,"每場完賽 +10")}${task("心情小棧留言","mood_message",15,"每則 +5")}<div class="ap-tabs"><button class="btn ${state.mode==='records'?'btn-primary':'btn-ghost'}" data-activity="records">我的積分紀錄</button><button class="btn ${state.mode==='daily'?'btn-primary':'btn-ghost'}" data-activity="daily">每日活躍榜</button><button class="btn ${state.mode==='cumulative'?'btn-primary':'btn-ghost'}" data-activity="cumulative">累積活躍榜</button><button class="btn btn-ghost" data-activity="refresh">重新整理</button></div>${state.loading?'<p>讀取中…</p>':table}</section>`;
 }
